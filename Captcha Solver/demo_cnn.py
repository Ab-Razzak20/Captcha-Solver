...
print("Predicting...")
prediction56 = [6 if arr[0] > 0.5 else 5 for arr in model56.predict(test_data)] # 5/6
prediction5 = model5.predict(test_data) # 5
prediction6 = model6.predict(test_data) # 6
...
print("5 or 6 model acc:{:.4f}%".format(correct56/total*100)) # 5/6
print("---------------------------")
print("5digits model acc:{:.4f}%".format(correct5/total5*100)) # 5 acc
for i in range(5):
    print("digit{:d} acc:{:.4f}%".format(i+1, correct5digit[i]/total5*100)) # 5 acc
print("---------------------------")
print("6digits model acc:{:.4f}%".format(correct6/total6*100)) # 6 acc
for i in range(6):
    print("digit{:d} acc:{:.4f}%".format(i+1, correct6digit[i]/total6*100)) # 6 acc
print("---------------------------")
print("alpha acc:{:.4f}%".format(correctalpha/totalalpha*100)) #  acc
