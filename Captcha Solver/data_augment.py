...
outputcsv = open('./data/6_real_train_set/captcha_train.csv', 'w', encoding = 'utf8', newline = '')
inputcsv = open('./data/manual_label/captcha_vali.csv', 'r', encoding = 'utf8')
data = [np.array(Image.open('./data/manual_label/' + row[0] + ".jpg")) for row in csv.reader(inputcsv) if len(row[1]) == 6]
inputcsv = open('./data/manual_label/captcha_vali.csv', 'r', encoding = 'utf8')
oldanswer = [row[1] for row in csv.reader(inputcsv) if len(row[1]) == 6] 
answer = []
