...
import csv
import numpy as np
FONTPATH = ["fonts/times-bold.ttf", "fonts/courier-bold.ttf"]
...
def my_generate(GENNUM, SAVEPATH, ENGP=25, FIVEP=0, ENGNOLIMIT=False, filename="train"):
    captchacsv = open(SAVEPATH + "captcha_{:s}.csv".format(filename), 'w', encoding = 'utf8', newline = '')
    lencsv = open(SAVEPATH + "len_{:s}.csv".format(filename), 'w', encoding = 'utf8', newline = '')
    letterlist = []
    lenlist = []
    for index in range(1, GENNUM + 1, 1):
        ...
        for i in range(captchalen):
            ...
            offset = newtext.next_offset
            captchastr += str(newtext.letter)
        letterlist.append([str(index).zfill(len(str(GENNUM))), captchastr.ljust(6, '*')])  #.ljust(6, '*')
        lenlist.append([str(index).zfill(len(str(GENNUM))), captchalen])
        for obj in rectlist:
            obj.draw(image=captcha, overlay=True)
        captcha.convert("RGB").save(SAVEPATH + str(index).zfill(len(str(GENNUM))) + ".jpg", "JPEG")
...
if __name__ == "__main__":
    print('0')
    # generate(10000, "C:/Users/tanvi/MyDatasets/data/56_imitate_train_set/",  ENGP=100, FIVEP=50, ENGNOLIMIT=True, filename="train")
    # print('1')
    # generate(2000, "C:/Users/tanvi/MyDatasets/data/56_imitate_vali_set/",  ENGP=100, FIVEP=50, ENGNOLIMIT=True, filename="vali")
    # print('2')
    # generate(1000, "C:/Users/tanvi/MyDatasets/data/5_imitate_train_set/",  ENGP=100, FIVEP=100, ENGNOLIMIT=True, filename="train")
    # print('3')
    # generate(200, "C:/Users/tanvi/MyDatasets/data/5_imitate_vali_set/",  ENGP=100, FIVEP=100, ENGNOLIMIT=True, filename="vali")
    # print('4')
    # generate(5000, "C:/Users/tanvi/MyDatasets/data/6_imitate_train_set/",  ENGP=100, FIVEP=0, ENGNOLIMIT=True, filename="train")
    # print('1')
    # generate(1000, "C:/Users/tanvi/MyDatasets/data/6_imitate_vali_set/",  ENGP=100, FIVEP=0, ENGNOLIMIT=True, filename="vali")

    my_generate(1000, "C:/Users/tanvi/MyDatasets/data/my_train_set/",  ENGP=100, FIVEP=0, ENGNOLIMIT=True, filename="train")
    print('1')
    my_generate(200, "C:/Users/tanvi/MyDatasets/data/my_vali_set/",  ENGP=100, FIVEP=0, ENGNOLIMIT=True, filename="vali")
